...
public class NoteSearchDataManagerTester 
{
	private	INoteSearchDataManager	dataManager;
	private	ApplicationContext	context;
	
	public	NoteSearchDataManagerTester()
	{
	...
	public	void	setUp()
	{
		dataManager	=	(NoteSearchDataManager)context.getBean("noteDataManager");
	}
	...
	public	void	testGetNoteForSearchWhenUserHasNotSearchAgainForKeywords()
	{
		...
		appUser.setId( "1" );
		dataManager.getNoteForSearch( appUser );
	}
	...
	public	void	testGetNoteForSearchWhenUserHasSearchAgainForKeywords()
	{
		...
		INoteDao	postDao	=	( HibernateNoteDao )context.getBean("noteDao");
		for( int postIndex = 1; postIndex < loopIterations; postIndex++ )
		{
			Note	note	=	new	Note();
			note.setId( String.valueOf( postIndex ) );
			note.setUser( user );
			postDao.Save( note );
			posts.add( note );
		}
		...
		Set<ISearchableFacebookFeed> results	=	(Set<ISearchableFacebookFeed>) dataManager.getNoteForSearch( appUser );
