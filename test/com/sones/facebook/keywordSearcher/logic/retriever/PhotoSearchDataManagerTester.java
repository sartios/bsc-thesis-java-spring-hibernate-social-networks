...
public class PhotoSearchDataManagerTester 
{
	private	IPhotoSearchDataManager	dataManager;
	private	ApplicationContext	context;
	
	public	PhotoSearchDataManagerTester()
	{
	...
	public	void	setUp()
	{
		dataManager	=	(PhotoSearchDataManager)context.getBean("photoDataManager");
	}
	...
	public	void	testGetVideoForSearchWhenUserHasSearchAgainForKeywords()
	{
		...
		IPhotoDao	postDao	=	( HibernatePhotoDao )context.getBean("photoDao");
		for( int postIndex = 1; postIndex < loopIterations; postIndex++ )
		{
			Photo	photo	=	new	Photo();
			photo.setId( String.valueOf( postIndex ) );
			photo.setUser( user );
			postDao.Save( photo );
			posts.add( photo );
		}
