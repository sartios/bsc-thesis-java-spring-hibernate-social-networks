...
public class FacebookNoteCapitalWordSpotterDao	extends	AbstractDao	implements	IFacebookFeedCapitalWordSpotterDao
{
	private	PropertiesConfiguration	_config;
	
	public	FacebookNoteCapitalWordSpotterDao()
	{
		super();
		try 
		{
			_config	=	new	PropertiesConfiguration("config/SqlScripts/FeedsWithCapitalizedWords/Facebook/note.properties");
		} 
	...
	private	List<ICapitalWordsSearchableFeed>	GetPictures( List<Object[]> objects )
	{
		...
		while(iterator.hasNext())
		{
			...
			ICapitalWordsSearchableContent	content	=	new	CapitalizedFacebookNote();
			((CapitalizedFacebookNote)content).SetMessage((String)object[_config.getInt( "message.index" )]);
			((CapitalizedFacebookNote)content).SetSubject((String)object[_config.getInt( "subject.index" )]);
