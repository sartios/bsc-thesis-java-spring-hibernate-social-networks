...
import com.sones.businessLogic.Facebook.Feeds.FeedListSorterFactory;
...
public class FacebookFeedList{
	 ...
	 */
	public FacebookFeedList(final String userID,final ArrayList<Feed> feeds){
		userID_ = userID;
		feeds_ = feeds;
	}
	...
	private List<Feed> getSortedFeedListNewestFirst() {
		if((null!=feeds_)&&(false==feeds_.isEmpty())){
			if(null==sorter_){
				sorter_=FeedListSorterFactory.getInstance().getSorter("quicksort");
			}
	...
	/**
	 * Finds which feeds contain the keyword and returns them
	 * @param keyword value
	 * @return List of feeds
	 */
	public List<Feed> getFeedsThatContainTheKeyword(final String keyword){
		List<Feed> results=null;
		if(null!=keyword){
			results = new ArrayList<Feed>();
			for(int i=0;i<this.getSize();i++){
				Feed feed = this.getFeed(i);
				if(feed.find(keyword)){
					results.add(feed);
				}
			}
		}
