...
public class FacebookSearchingManager extends TimerTask{
	...
	public void performSearch(){
		...
		if(null!=sources_){
			...
			while(null!=(currentUserID=sources_.getID())){
				FeedList feeds = handler_.getFeedsSinceDate(currentUserID, token_.getToken(),this.getSinceDateInUnixTimestamp());
				if(!feeds.isEmpty()){
					searcher_.setFeeds(feeds);
					String newerFeedDate=String.valueOf(dateConverter.getUnixTimestamp(feeds.getNewerFeed().getCreatedTime_()));
	...
	public void setSinceDateInUnixTimestamp(String unixTimeStamp) {
		if(null!=unixTimeStamp){
			this.sinceDateInUnixTimestamp = Long.parseLong(unixTimeStamp);
		}
	}
