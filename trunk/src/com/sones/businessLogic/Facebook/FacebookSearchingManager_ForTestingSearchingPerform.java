...
/**
 * This class is a copy of FacebookSearchingManager
 * for testing purposes. We have a FeedList in which
 * we keep the feeds for testing.
 * 
 * 
 * @author Sartios
 *
 */
public class FacebookSearchingManager_ForTestingSearchingPerform extends TimerTask{
	 ...
	 */
	private AbstractSearcher keywordSearcher_ = null;
	...
	/**
	 * The testing feeds in which we want to search
	 */
	private FeedList feeds_;
	
	
	private CommentTrafficSearcher commentTrafficSearcher=new CommentTrafficSearcher();
	
	/**
	 * In the constructor, we initialize the {@link #token_} to be the fist of Table.
	 */
	public FacebookSearchingManager_ForTestingSearchingPerform(){
	 ...
	 * @version 1.0
	 */
	public FacebookSearchingManager_ForTestingSearchingPerform(final String sinceDate){
		TokenDao dao = new TokenDao();
		token_ = (FacebookToken) dao.findAll().get(0);
		feedDao_ = new FeedDao();
		handler_ = new FacebookRestHandler();
	...
	public void performSearch(){
		...
		//if(null!=sources_){
			...
			//while(null!=(currentUserID=sources_.getID())){
				...
				if(!feeds_.isEmpty()){
					keywordSearcher_.setFeeds(feeds_);
					String newerFeedDate=(feeds_.getNewerFeed().getCreatedTime_());
					this.setSinceDateInUnixTimestamp(newerFeedDate);
					keywordSearcher_.search();
					commentTrafficSearcher.setFeeds(feeds_);
					commentTrafficSearcher.search();
