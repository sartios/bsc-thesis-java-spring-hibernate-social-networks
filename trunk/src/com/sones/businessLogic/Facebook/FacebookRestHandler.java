...
public class FacebookRestHandler {
	...
	public FacebookGroupList getGroups(final String ID){
		...
		if(null!=ID){
			TokenDao dao = new TokenDao();
			FacebookToken token = (FacebookToken)dao.findAll().get(0);
			jsonString = doGet("https://graph.facebook.com/"+ID+"/groups/?"+token.getToken());
		}
	...
	public FeedList getFeeds(final String ID,final String TOKEN){
		...
		if((null!=ID)&&(null!=TOKEN)){
			...
			FacebookJsonHandler jsonHandler = new FacebookJsonHandler();
			feeds.setUserID(ID);
			feeds.setFeeds(jsonHandler.getFeeds(jsonString));
		}
		return feeds;
	}
	...
	public Feed getFeed(final String FEED_ID) {
		...
		if(null!=FEED_ID){
			jsonString=doGet("https://graph.facebook.com/"+FEED_ID+"?"+token_.getToken());
		}
		return jsonHandler.getFeed(jsonString);
	}
	...
	public FacebookFriend getFacebookUser(final String userID){
		...
		String jsonString=null;
		if(null!=userID){
			jsonString=doGet("https://graph.facebook.com/"+userID+"?"+token_.getToken());
		}
	 ...
	 * @return FeedList
	 */
	public FeedList getFeedsSinceDate(final String ID,final String TOKEN,final long date){
		FeedList feeds=new FeedList();
		feeds.clearFeeds();
		//if(date>0){
