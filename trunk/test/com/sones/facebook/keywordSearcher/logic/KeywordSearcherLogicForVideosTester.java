...
import com.sones.facebook.dao.hibernate.feed.HibernateVideoDao;
...
import com.sones.facebook.keywordSearcher.logic.retriever.VideoSearchDataManager;
...
public class KeywordSearcherLogicForVideosTester 	extends	AbstractKeywordSearcherLogicTester	implements	IKeywordSearcherServiceTester
{
	...
	public	KeywordSearcherLogicForVideosTester()
	{
		super();
		videoDao	=	( HibernateVideoDao )getContext().getBean( "videoDao" );
		searcherService	=	( KeywordSearcherService )getContext().getBean( "keywordSearcherService" );
		dataManager	=	( VideoSearchDataManager )getContext().getBean("videoDataRetriever");
	...
	public	void	testSearchForKeywordsWhenThereAreNotPostsThatContainTheKeywords()
	{
		...
		Iterable< FacebookPost >	posts	=	SaveVideosThatDoNotContainKeywordsAndReturn();
		...
		searcherService.searchForKeywordsIntoVideos(appUser, date);
	...
	private	Iterable<FacebookPost>	SaveLinksThatContainKeywordsAndReturn()
	{
		...
		for( int postIndex = 1; postIndex < 6; postIndex++ )
		{
			...
			post.setDescription(MESSAGE+ getKeywordValue() + postIndex );
			videoDao.Save( post );
			posts.add( post );
		}
		return	posts;
	...
	private	Iterable<FacebookPost>	SaveLinksThatSomeContainKeywordsAndReturn()
	{
		...
		for( int postIndex = 4; postIndex < 9; postIndex++ )
		{
			Video	post	=	new	Video();
			post.setId( String.valueOf( postIndex ) );
			post.setDescription(MESSAGE + postIndex );
			videoDao.Save( post );
			posts.add( post );
		}
		return	posts;
	...
	private	Iterable<FacebookPost>	SaveVideosThatDoNotContainKeywordsAndReturn()
	{
		List< FacebookPost >	posts	=	new	ArrayList<FacebookPost>();
		for( int postIndex = 1; postIndex < 6; postIndex++ )
		{
